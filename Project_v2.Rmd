...
ggplot(data = alarm, 
       mapping = aes(x= Hour)) +
  geom_histogram(color = 'purple', fill = 'pink') + labs(title = "Number of calls at each Hour")+
  guides(fill = 'none')
...
incident <- fire %>% group_by(Inc_des) %>% summarize(count = n()) %>% arrange(desc(count)) %>% filter(count > 90)

ggplot(data = fire, mapping = aes(x = Inc_des, fill = Inc_des))+
  geom_bar()+theme(axis.text.y = element_text(size = 5))+  # We have an issue here
  scale_x_discrete(limits = incident$Inc_des)+guides(fill='none') + coord_flip() + xlab("Incident Description")
...
shift_time <- fire %>% filter(!is.na(Shift)) %>% filter(!is.na(Dis_dt)) %>%
